...
(defun pie-chart (slices)
  ...
  (let* ((base (md5-as-string (md5sum-string (format nil "~{~a~}" (mapcar #'car slices)))))
         (file (format nil "~~/quicklisp/local-projects/tacit-tales/static/images/~a.png" base)))
    (with-canvas (:width 400 :height 250)
      (set-rgb-fill 1.0 0 0)
      (centered-circle-path 105 125 100)
      (fill-path)
      (set-font (get-font "~/quicklisp/local-projects/tacit-tales/fonts/kenpixel.ttf") 10)
      (let ((ratios slices)
